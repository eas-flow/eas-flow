...
**ステータス:** 開発初期（`v0.1.0` リリース済み、`v0.2.0` 作業中）。API・構成は変更される可能性があります。
...
| `develop-work`  | v0.2.0     | イシューから統合ブランチ基点で作業ブランチを作成し、`explorer` サブエージェントに調査を委任、品質ゲートを通してPRを作成 |
| `doc-sync`      | v0.2.0     | 実装とドキュメントの乖離を `doc-auditor` サブエージェントに委任して検出し、承認後に更新案を反映                         |
...
Lint・型チェックの通過を必須とし、影響の大きい操作（実装方針の判断・ドキュメント編集・ビルド・
バージョン更新・Release作成）の前にはユーザー承認を求めます。
